function varargout=wtc_mc(x,y,varargin)
%% Wavelet coherence
%
% USAGE: [Wxy,Rsq,period,scale,coi,sig95]=wtc(x,y,[,settings])
%
% 
...
Args=struct('Pad',1,...      % pad the time series with zeroes (recommended)
            ...
            'S0',[],...    % this says start at a scale of 8 sec (0.125Hz)
            'J1',[],...
            'Mother','Morlet', ...
...
if isempty(Args.J1)
    if isempty(Args.MaxScale)
        Args.MaxScale=128; %auto maxscale
    end
...
if (nargout>0)||(Args.MakeFigure)
    wtcsig=wtcsignif(Args.MonteCarloCount,Args.AR1,dt,Args.Pad,Args.Dj,Args.S0,Args.J1,Args.Mother);
